...
type RestOf<T extends ICard | NewCard> = Omit<T, 'timeGenerated'> & {
...
export class CardService {
  ...
  protected convertDateFromClient<T extends ICard | NewCard | PartialUpdateCard>(card: T): RestOf<T> {
    return {
      ...card,
      timeGenerated: card.timeGenerated?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCard: RestCard): ICard {
    return {
      ...restCard,
      timeGenerated: restCard.timeGenerated ? dayjs(restCard.timeGenerated) : undefined,
    };
  }
