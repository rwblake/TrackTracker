...
public class CardResource {
    ...
    public ResponseEntity<Card> partialUpdateCard(
        ...
        Optional<Card> result = cardRepository
            .findById(card.getId())
            .map(existingCard -> {
                if (card.getMetric() != null) {
                    existingCard.setMetric(card.getMetric());
                }
                if (card.getTimeFrame() != null) {
                    existingCard.setTimeFrame(card.getTimeFrame());
                }
                if (card.getMetricValue() != null) {
                    existingCard.setMetricValue(card.getMetricValue());
                }
                if (card.getTimeGenerated() != null) {
                    existingCard.setTimeGenerated(card.getTimeGenerated());
                }
